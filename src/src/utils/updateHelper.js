...
export default SYNOCOMMUNITY.RRManager.UpdateHelper = {
  ...
  updateFileInfoHandler: function (fileInfo) {
    ...
    fileInfo = shareRealPath.replace(shareName, fileInfo.path.slice(1));
    this.apiProvider
      .callCustomScript(`uploadUpdateFileInfo.cgi?file=${encodeURIComponent(fileInfo)}`)
      .then(() => {
  ...
  onRunRrUpdateManuallyClick: function (updateFilePath) {
    ...
    function runUpdate() {
      self.apiProvider
        .getUpdateFileInfo(updateFilePath)
        .then(responseText => {
          if (!responseText.success) {
            self.helper.unmask(self.owner);
            self.showMsg(
              self.helper.formatString(
                self.helper.V('upload_file_dialog', 'unable_update_rr_msg'),
          ...
          async function runUpdate() {
            ...
            const updateStatusInterval = setInterval(async function () {
              const checksStatusResponse = await self.apiProvider.callCustomScript(
              ...
              self.helper.mask(
                self.appWin,
                self.helper.formatString(
                  self.helper.V('upload_file_dialog', 'update_rr_progress_msg'),
                  response?.progress ?? '--',
          ...
          self.appWin.getMsgBox().confirmDelete(
            'Confirmation',
            self.helper.formatString(
              self.helper.V('upload_file_dialog', 'update_rr_confirmation'),
              currentRrVersion,
              updateRrVersion
            ),
            userResponse => {
